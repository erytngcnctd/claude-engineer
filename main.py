...
def create_eth_wallet(wallet_name=None):
    ...
    try:
        ...
        if wallet_name in wallets:
            raise ValueError(f"Wallet with name '{wallet_name}' already exists")
        
        wallets[wallet_name] = wallet_data
        
        with open(wallets_file, 'w') as f:
            json.dump(wallets, f, indent=2)
        
        # Ensure the wallet file has restricted permissions
...
def list_eth_wallets():
    ...
    try:
        if os.path.exists(wallets_file):
            with open(wallets_file, 'r') as f:
                wallets = json.load(f)
            return [{"name": name, "address": data["address"]} for name, data in wallets.items()]
        else:
            return []
    except Exception as e:
...
def get_eth_wallet_details(wallet_name):
...
tools = [
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                "old_name": {
                    "type": "string",
                    "description": "The current name of the wallet"
                },
                "new_name": {
                    "type": "string",
    ...
    {
        "name": "list_eth_wallets",
        "description": "List all stored Ethereum wallets.",
        "input_schema": {
            "type": "object",
            "properties": {}
        }
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                "from_wallet_name": {
                    "type": "string",
                    "description": "The name of the stored wallet to send from"
                },
                "to_address": {
                    "type": "string",
    ...
    {
        "name": "get_balance",
        "description": "Get the balance of a stored Ethereum wallet.",
        "input_schema": {
            "type": "object",
            "properties": {
                "wallet_name": {
...
def rename_wallet(old_name, new_name):
    ...
    try:
        ...
        if old_name not in wallets:
            raise ValueError(f"Wallet '{old_name}' does not exist")
        
        if new_name in wallets:
            raise ValueError(f"Wallet name '{new_name}' already exists")
        ...
        return f"Wallet renamed from '{old_name}' to '{new_name}'"
    except Exception as e:
        logging.error(f"Error renaming wallet: {str(e)}")
        raise
...
async def execute_tool(tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        elif tool_name == "list_eth_wallets":
            wallets = list_eth_wallets()
            result = "Stored Ethereum wallets:\n" + "\n".join([f"Name: {w['name']}, Address: {w['address']}" for w in wallets])
        elif tool_name == "get_eth_wallet_details":
            wallet = get_eth_wallet_details(tool_input["wallet_name"])
            if wallet:
                result = f"Wallet details:\nName: {wallet['name']}\nAddress: {wallet['address']}\nPrivate Key: {wallet['private_key']}"
            else:
                result = f"No wallet found with name: {tool_input['wallet_name']}"
                is_error = True
        elif tool_name == "send_eth":
            result = send_eth(
